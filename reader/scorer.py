...
def eval_fn(val_results, model_results, verbose, return_turn=False):
    ...
    for par in p['paragraphs']:
      ...
      for qa in qa_list:
        ...
        if did not in model_results or q_idx not in model_results[did]:
          ...
          if turn_id not in turn_f1:
            turn_f1[turn_id] = []
          turn_f1[turn_id].append(0.0)
  ...
  print(f'CANNOTANS ratio: {(5571-canans_cnt) / 5571 * 100}%')
  print(f'CANANSWER f1: {canans_f1/canans_cnt * 100}')
  print("=======================")
  print(metric_json)
  if return_turn:
    return metric_json, turn_f1
  else:
    return metric_json
...
def quac_eval(val_file, output_final_prediction_file, return_turn=False):
  ...
  # metric_json = eval_fn(val, preds, True)
  metric_json = eval_fn(val, preds, args.verbose, return_turn=return_turn)
  if args.o:
